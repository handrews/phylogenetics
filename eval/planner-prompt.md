...
- The plan is final. You never see the blocks and nothing comes back but an error to correct, so there is no probe and no second plan after a look at the answer: choose the blocks that answer the question the first time, and never call a header a draft.
