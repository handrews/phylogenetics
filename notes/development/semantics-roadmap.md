...
- `errors: [fields]` (or `true` for the whole line): what is printed is
  wrong, and `corrections`, a JSON Merge Patch over the node, gives what
  the editor reads instead (a `null` deletes a printed field). A citation
  that cannot resolve as printed (in press, wrong year, a page from
  another printing) is the first case: the printed `auth`/`year`/`citedAs`
  stay, the errors name them, and the corrections set
...
record is kept as printed and the disagreement is derived (B19); `errors`
is for what the editor judges wrong on the page, with the evidence in
`basis`.
...
`errors` is that editor-verified marker for a discrepancy that is the
source's own error, so verified and unverified discrepancies can be told
apart.
...
audit:
  ...
  coverage:
    ...
    synonymy: partly
    material: none
  notes: "Reviewed against the paper 2026-09-10; see notes/reviews/…"
```
...
ingestion work, is `notes/development/plan.md`; read that first. Step 0 is done.
...
0b. **Data corrections from the reading round.** The table at the end of
   `notes/audits/source-observations.md` ("Data corrections surfaced by this
