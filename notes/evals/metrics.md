...
note re-grades runs 2 to 5 with the final grader over the final 47
...
prompt gap fixed since, which the write-up ties to six of its thirteen
misses; a fresh run is needed before the figure is quoted.
...
| answer summarises or passes a verdict in its own words (judge score 0) | 18 | 2 | 1 | 2 | 0 |
| internal names leak into the answer | 1 | 0 | 0 | 0 | 0 |
| lookup limit reached before answering | 9 | 4 | 3 | 3 | 0 |
| wrong or missing block | – | 12 | 8 | 5 | 12 |
| expected wording missing from the rendered answer | – | 21 | 8 | 5 | 11 |
| commentary written beside the answer (reaches no reader) | – | 11 | 5 | 12 | 2 |
...
words shrink to a one-line header. The lookup limit stops mattering
when the model plans instead of reading. The wording row falls from
...
and 5 is the model choosing a block the question did not ask for.
